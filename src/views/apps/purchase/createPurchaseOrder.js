...
import AddNewVendorForm from "../../../components/CommonForm/add-vendor";
import { getEditVendor, getVendorForDifferentUser } from "../../../api/vendor";
...
const Index = () => {
  ...
  const [getVendorOpen, setVendorOpen] = useState(false);
  ...
  const [getVendorTagList, setVendorTagList] = useState([]);
  ...
  useEffect(() => {
    getVendorID && getVendorInfoHandler(getVendorID);
  }, [getVendorID]);
        ...
        <div className="card-sub-div">
          ...
          >
            <h3
              ...
              onClick={() => navigate("/apps/purchases-order/list")}
            >
              <ChevronLeft />
            ...
            <div className="inv-text">
              <span>
                <Input
                  type="text"
                  value={`${settingDetail?.purchase_order_prefix}`}
                  className="inv-text"
                />
              </span>
              <span>
          ...
          <div className="invoice-card-box">
            <Row>
              <Col className="mb-1" xl="4" md="6" sm="12">
                ...
                <Select
                  onChange={(value) => {
                    if (!getVendorTagList.includes(value)) {
                      setVendorTagList((prev) => [...prev, value]);
                    } else {
                      message.error("Vendor alreday added");
                    }
                  }}
                ...
                >
                  {vendorList?.map((item) => (
                    <Select.Option value={item.vendor_id}>
                      <div className="customer_name_phone_amount_div">
                        <span className="customer_name_phone">
                          {item?.vendor_name}({item?.vendor_phone})
                        </span>
                        {item?.vendor_debit_amount ? (
                ...
                {vendorList &&
                  vendorList.map((val) => {
                    return (
                      getVendorTagList.includes(val?.vendor_id) && (
                        <span className="customer-tag">
                          <span
                            className="customer-for-user"
                            onClick={() => setVendorID(val?.vendor_id)}
                          >
                            {val?.vendor_name}
                          </span>
                          <span
                            ...
                            onClick={() => {
                              setVendorTagList(
                                getVendorTagList.filter(
                                  (value) => value !== val?.vendor_id
                                )
                              );
                            }}
                          >
